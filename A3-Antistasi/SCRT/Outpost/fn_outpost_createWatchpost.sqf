...
if(_isRoad) exitWith {
	...
	["Watchpost", "Watchpost can't be on road."] call A3A_fnc_customHint;
};
...
if ((_resourcesFIA < _costs) or (_hrFIA < _hr)) exitWith {
    playSound "3DEN_notificationWarning";
	["Watchpost", format ["You lack of resources to build this Watchpost <br/><br/> %1 HR and %2 € needed",_hr,_costs]] call A3A_fnc_customHint;
};
...
[[teamPlayer,civilian],"outpostTask",["We are sending a team to establish a watchpost. Use HC to send the team to their destination","Post \ Watchpost Deploy",_marker],_position,false,0,true,"Move",true] call BIS_fnc_taskCreate;
_formatX = [];
{
