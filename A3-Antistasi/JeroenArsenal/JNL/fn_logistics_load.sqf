...
if _playAnimation then{
	...
	[_vehicle,_object] spawn {
		...
		*/
		...
		sleep 0.1;
		[_vehicle] remoteExec ["jn_fnc_logistics_lockSeats",[0,-2] select isMultiplayer,_vehicle];
...
{
	[_vehicle] remoteExec ["jn_fnc_logistics_addActionUnload",[0, -2] select isMultiplayer,_vehicle];
};

//Add getOut event hanldler and getin Action
if(_objectType == 0) then
{
	[_object] remoteExec ["jn_fnc_logistics_addEventGetoutWeapon",[0, -2] select isMultiplayer,_object];

	[_vehicle,_object] remoteExec ["jn_fnc_logistics_addActionGetinWeapon",[0, -2] select isMultiplayer,_vehicle];
};
