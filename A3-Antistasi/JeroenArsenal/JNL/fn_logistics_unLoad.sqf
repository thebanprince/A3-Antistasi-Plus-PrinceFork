...
if(_nodeLast == 0)then{
	[_vehicle] remoteExec ["jn_fnc_logistics_removeActionUnload",[0, -2] select isMultiplayer, _vehicle];
	[_vehicle] remoteExec ["jn_fnc_logistics_removeActionGetInWeapon", [0, -2] select isMultiplayer, _vehicle];
	[_object] remoteExec ["jn_fnc_logistics_removeEventGetOut", [0, -2] select isMultiplayer, _object];
};
