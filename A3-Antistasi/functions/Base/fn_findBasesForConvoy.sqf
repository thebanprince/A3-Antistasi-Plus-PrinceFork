...
_airportsAAF = (airportsX + outposts + milbases) select {(spawner getVariable _x == 2) and (dateToNumber date > server getVariable _x) and ([_x,_markerX] call A3A_fnc_isTheSameIsland) and (!(_x in forcedSpawn)) and (sidesX getVariable [_x,sideUnknown] == _sideX) and !(_x in blackListDest) and (getMarkerPos _x distance _pos > distanceSPWN)};
