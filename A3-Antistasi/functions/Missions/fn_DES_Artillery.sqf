...
private _timeLimit = 1 * settingsTimeMultiplier;
...
switch(true) do {
    ...
    case (!(alive _artilleryVeh) || _artilleryCrew findIf {alive _x} == -1): {
        ...
        { [500,_x] call A3A_fnc_playerScoreAdd } forEach (call BIS_fnc_listPlayers) select { side _x == teamPlayer || side _x == civilian};
        [15, theBoss] call A3A_fnc_playerScoreAdd;
    };
    default {
