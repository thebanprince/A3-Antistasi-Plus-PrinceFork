...
if (str (_display) != "no display") then
{
	...
	_ChildControl  ctrlSetTooltip format [localize "STR_antistasi_dialogs_generic_costs_generix_text",server getVariable (SDKEng select 0)];
	_ChildControl = _display displayCtrl 108;
	_ChildControl  ctrlSetTooltip format [localize "STR_antistasi_dialogs_generic_costs_generix_text",server getVariable (SDKExp select 0)];
