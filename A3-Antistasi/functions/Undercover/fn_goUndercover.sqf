...
_secureBases = airportsX + outposts + seaports + _roadblocks + milbases;
...
if ((_player distance getMarkerPos _base < _size * 2) and(not(sidesX getVariable[_base, sideUnknown] == teamPlayer))) exitWith {
	["Undercover", "You cannot go Undercover near Airports, Outposts, Seaports, Military Bases or Roadblocks"] call A3A_fnc_customHint;
};
